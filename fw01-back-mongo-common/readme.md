...
- **API**: gRPC (Spring gRPC Server v0.11.0, protoc-gen-grpc-java v1.74.0, Protobuf v4.31.1)
- **Build Tool**: Maven
...
Microservice #1 backend (synchronous edition) providing a gRPC interface for a dictionary service.

### gRPC Service (`DicService` on port 9090):
...
Run with: `mvn spring-boot:run` (requires `CERTIFICATE_PASSWORD` env var for mTLS).